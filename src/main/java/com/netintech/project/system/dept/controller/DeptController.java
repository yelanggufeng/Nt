package com.netintech.project.system.dept.controller;
...
public class DeptController extends BaseController
{
    ...
    public AjaxResult addSave(@Validated Dept dept)
    {
        ...
        if(dept.getParentId()==Long.valueOf(configdeptId)){
            return AjaxResult.warn("不能选择该节点");
        }
    ...
    public AjaxResult editSave(@Validated Dept dept)
    {
        ...
        if(dept.getParentId()==Long.valueOf(configdeptId)){
            return AjaxResult.warn("不能选择该节点");
        }else if(dept.getDeptId()==Long.valueOf(configdeptId) && "1".equals(dept.getStatus())) {
            return AjaxResult.warn("不能停用该节点");
        }
    ...
    public AjaxResult remove(@PathVariable("deptId") Long deptId)
    {
        ...
        if(deptId==Long.valueOf(configdeptId)){
            return AjaxResult.warn("不能删除该节点");
        }
        if(deptService.countParentById(deptId)>0){
            return AjaxResult.warn("不能删除该节点");
        }
     ...
     * 选择部门树
     */
    @GetMapping("/selectGovDeptTree/{deptId}")
    public String selectGovDeptTree(@PathVariable("deptId") Long deptId, ModelMap mmap)
    {
        mmap.put("dept", deptService.selectDeptById(deptId));
        return prefix + "/govtree";
    }

    /**
     * 加载政府部门列表树
     */
    @GetMapping("/govtreeData")
    @ResponseBody
    public List<Ztree> govtreeData()
    {
        List<Ztree> ztrees = deptService.selectGovDeptTree(new Dept());
        return ztrees;
    }
