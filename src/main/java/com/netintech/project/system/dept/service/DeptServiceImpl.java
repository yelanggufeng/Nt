package com.netintech.project.system.dept.service;
...
public class DeptServiceImpl implements IDeptService
{
    ...
    @Override
    public List<Ztree> selectGovDeptTree(Dept dept)
    {
        dept.setDeptId(Long.valueOf(configdeptId));
        List<Dept> deptList = deptMapper.selectGovDeptList(dept);
        List<Ztree> ztrees = initZtree(deptList);
        return ztrees;
    }
    ...
    public int insertDept(Dept dept)
    {
        ...
        dept.setCreateBy(ShiroUtils.getUserStringId());
        dept.setAncestors(info.getAncestors() + "," + dept.getParentId());
        if(StringUtils.isEmpty(dept.getDeptType())) {
            dept.setDeptType(info.getDeptType());
            if (info.getDeptType().equals("0")) {
                dept.setDeptType("1");
            } else if (info.getDeptId() == Integer.parseInt(configdeptId)) {
                dept.setDeptType("2");
            }
        }
    ...
    public int updateDept(Dept dept)
    {
        ...
        if (StringUtils.isNotNull(newParentDept) && StringUtils.isNotNull(oldDept))
        {
         ...
         dept.setDeptType(newParentDept.getDeptType());
        dept.setDeptType(newParentDept.getDeptType());
        if(newParentDept.getDeptType().equals("0")){
            dept.setDeptType("1");
        }else if(newParentDept.getDeptId()==Integer.parseInt(configdeptId)){
            dept.setDeptType("2");
        }
        dept.setUpdateBy(ShiroUtils.getUserStringId());
    ...
    @Override
    public void updateDeptNameByID(SmallPark sp){
         deptMapper.updateDeptNameByID(sp);
    }

    @Override
    public  int countParentById(Long id){
        return deptMapper.countParentById(id);
    }

    @Override
    public Long getPriseIdByDeptId(Long child) {
        Long deptId =child;
        String str =  userMapper.getUserDeptId(child);
        String[] list = str.split(",");
        if(list.length>4){
            deptId = Long.valueOf(list[4]);
        }
        return deptId;
    }

    @Override
    public Long selectParentById(Long dept) {
        return deptMapper.selectParentById(dept);
    }
