package com.netintech.process.definition.controller;
...
import com.netintech.process.definition.domain.ProcessDefinition;
import com.netintech.process.definition.service.ProcessDefinitionService;
    ...
    public AjaxResult upload(@RequestParam("processDefinition") MultipartFile file) {
        try {
            if (!file.isEmpty()) {
                ...
                String fileName = FileUploadUtils.upload(NtConfig.getProfile() + "/processDefiniton", file);
                if (StringUtils.isNotBlank(fileName)) {
                    String realFilePath = NtConfig.getProfile() + fileName.substring(Constants.RESOURCE_PREFIX.length());
