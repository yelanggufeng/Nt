...
(function ($) {
    $.extend({
    	...
    	table: {
            ...
            // 导出excel数据
            exportExcelData: function(exportUrl) {
                table.set();
                $.modal.confirm("确定导出所有" + table.options.modalName + "吗？", function() {
                    $.modal.loading("正在导出数据，请稍后...");
			...
			// 回显数据字典
			selectKey: function(datas, value) {
				var actions = [];
				$.each(datas, function(index, data) {
					if (data.value == ('' + value)) {
						var listClass = $.common.equals("default", data.listClass) || $.common.isEmpty(data.listClass) ? "" : "badge badge-" + data.listClass;
						actions.push($.common.sprintf("<span class='%s'>%s</span>", listClass, data.key));
						return false;
					}
				});
				return actions.join('');
    	...
    	modal: {
            ...
            // 弹出提示2秒自动关闭
            alertClose: function(content, type, time) {
                layer.alert(content, {
                    icon: $.modal.icon(type),
                    title: "系统提示",
                    btn: ['确认'],
        ...
        operate: {
			...
			// 评价
			evaluate: function(id, width, height) {
            	table.set();
            	var _url = $.operate.evaluateUrl(id);
            	var _width = $.common.isEmpty(width) ? "800" : width;
                var _height = $.common.isEmpty(height) ? ($(window).height() - 50) : height;
            	//如果是移动端，就使用自适应大小弹窗
            	if (navigator.userAgent.match(/(iPhone|iPod|Android|ios)/i)) {
            ...
            // 详细访问地址
			evaluateUrl: function(id) {
            	var url = "/404.html";
            	if ($.common.isNotEmpty(id)) {
            	    url = table.options.evaluateUrl.replace("{id}", id);
            	} else {
            	    var id = $.common.isEmpty(table.options.uniqueId) ? $.table.selectFirstColumns() : $.table.selectColumns(table.options.uniqueId);
            	    if (id.length == 0) {
            			$.modal.alertWarning("请至少选择一条记录");
			...
			// 添加信息
			apply: function(id,width,height) {
				table.set();
				$.modal.open("添加" + table.options.modalName, $.operate.applyUrl(id),width,height);
			},
			// 申报项目
			apply: function(id,width,height,titile) {
				table.set();
				$.modal.open(titile, $.operate.applyUrl(id),width,height);
			},
            // 添加信息，以tab页展现
            addTab: function (id) {
            	table.set();
			...
			// 添加访问地址
			applyUrl: function(id) {
				var url = $.common.isEmpty(id) ? table.options.applyUrl.replace("{id}", "") : table.options.applyUrl.replace("{id}", id);
				return url;
			},
            // 修改信息
            edit: function(id) {
			...
			edit_size: function(id,a,b) {
				...
				if($.common.isEmpty(id) && table.options.type == table_type.bootstrapTreeTable) {
					...
					var url = table.options.updateUrl.replace("{id}", row[table.options.uniqueId]);
					$.modal.open("修改" + table.options.modalName, url,a,b);
				} else {
					$.modal.open("修改" + table.options.modalName, $.operate.editUrl(id),a,b);
				}
			},
			// 修改信息
			edittemp: function(id,a,b) {
				table.set();
				if($.common.isEmpty(id) && table.options.type == table_type.bootstrapTreeTable) {
					var row = $("#" + table.options.id).bootstrapTreeTable('getSelections')[0];
					if ($.common.isEmpty(row)) {
            ...
            successTabCallback: function(result) {
                if (result.code == web_status.SUCCESS) {
    	            ...
    	            var $contentWindow = $('.Nt_iframe[data-id="' + currentId + '"]', topWindow)[0].contentWindow;
    	            $.modal.close();
        ...
        tree: {
			...
			getCheckedChildNodes: function(column) {
				...
				for(var i in nodes) {
					...
					keyArray.push({id:nodes[i].id, name:nodes[i].name})
				}
				// return $.map(nodes2, function (row) {
				// 	return row[_column];
				// });
